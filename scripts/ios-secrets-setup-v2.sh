...
# Based on successful approach with vue3-starter project
...
OUTPUT_FILE="$TEMP_DIR/github_secrets.txt"
{
    ...
    echo "# Here's a simplified workflow example that works with these secrets:"
    echo "name: Build iOS App"
    echo "on: [push, workflow_dispatch]"
    echo "jobs:"
    echo "  build-ios:"
    echo "    runs-on: macos-latest"
    echo "    steps:"
    echo "      - uses: actions/checkout@v3"
    echo "      - name: Set up development certificate"
    echo "        uses: apple-actions/import-codesign-certs@v2"
    echo "        with:"
    echo "          p12-file-base64: \${{ secrets.IOS_DEVELOPMENT_CERTIFICATE }}"
    echo "          p12-password: \${{ secrets.IOS_DEVELOPMENT_CERTIFICATE_PASSWORD }}"
    echo "          keychain-password: \${{ github.run_id }}"
    echo "      - name: Install provisioning profile"
    echo "        run: |"
    echo "          mkdir -p ~/Library/MobileDevice/Provisioning\\ Profiles"
    echo "          echo -n \"\${{ secrets.IOS_ADHOC_PROVISIONING_PROFILE }}\" | base64 --decode > ~/Library/MobileDevice/Provisioning\\ Profiles/profile.mobileprovision"
    echo "      # Configure identity in your build command"
    echo "      # Usually with something like: -allowProvisioningUpdates -teamId \${{ secrets.APPLE_TEAM_ID }}"
} > "$OUTPUT_FILE"
